...
- `showUpdateDialog()` / `run()` now render the built-in `UpdateRequiredDialog` and manage the whole flow internally — the "Remind Me Later" snooze is applied without any caller-side wiring.
- `run()` reuses the existing config when called without `android`/`ios` (e.g. after `init()` at startup), so a single `EasyUpdate.instance.run(context)` is enough.

### Added

- `closeAppOnAndroidUpdate` (default `true`) on `run()` / `showUpdateDialog()` — closes the app on Android after sending the user to the store, so Play installs the update cleanly.
- Public `remindLater()` to trigger the snooze programmatically.
